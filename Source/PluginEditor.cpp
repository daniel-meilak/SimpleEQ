...
void LookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height, float sliderPosProportional, float rotaryStartAngle,
...
void ResponseCurveComponent::paint(juce::Graphics& g)
{
    ...
    for (int i = 0; i < w; ++i) {
        ...
        double mag = 1;
        auto freq = mapToLog10(static_cast<double>(i) / static_cast<double>(w), 20.0, 20'000.0);
    ...
    highCutSlopeSliderAttachment(audioProcessor.apvts, "HighCut Slope", highCutSlopeSlider)
{
    ...
    lowCutFreqSlider.labels.add({ 1.f, "20Hz" });

    highCutFreqSlider.labels.add({ 0.f, "20Hz" });
    highCutFreqSlider.labels.add({ 1.f, "20Hz" });
